...
export function WorkPage() {
  ...
  async function increasePriority(kind:'task'|'work', id:string, currentPriority:string){
    if(!currentCompany)return
    const table=kind==='task'?'tasks':'work_items'
    const next=currentPriority==='urgent'?'normal':currentPriority==='high'?'urgent':'high'
    const{error:err}=await supabase.from(table).update({priority:next}).eq('id',id).eq('company_id',currentCompany.id)
    if(err)setError(err.message);else await load()
  }
  ...
  return <section>
    ...
    <div className="panel-card no-pad">{loading?<div className="loading-block">Carregando...</div>:visibleTasks.length+visibleWork.length?<div className="record-list">
      {visibleTasks.map(i=><WorkRow key={`t-${i.id}`} title={i.title} detail={i.description} due={i.due_at} priority={i.priority??'normal'} kind="Tarefa" contact={contactName(i.contact_id)} person={i.person_name} status={i.status} onComplete={()=>void complete('task',i.id)} onPostpone={()=>void postponeOneDay('task',i.id,i.due_at)} onPrioritize={()=>void increasePriority('task',i.id,i.priority??'normal')} onEdit={()=>openEditTask(i)} onDelete={()=>void remove('task',i.id)}/>) }
      {visibleWork.map(i=><WorkRow key={`w-${i.id}`} title={i.title} detail={i.description} due={i.due_at} priority={i.priority} kind={workTypes.find(t=>t.v===i.type)?.l??i.type} contact={contactName(i.contact_id)} person={i.person_name} amount={formatMoney(i.amount)} status={i.status} onComplete={()=>void complete('work',i.id)} onPostpone={()=>void postponeOneDay('work',i.id,i.due_at)} onPrioritize={()=>void increasePriority('work',i.id,i.priority)} onEdit={()=>openEditWork(i)} onDelete={()=>void remove('work',i.id)}/>) }
...
function WorkRow({title,detail,due,priority,kind,contact,person,amount,status,onComplete,onPostpone,onPrioritize,onEdit,onDelete}:{title:string,detail?:string|null,due?:string|null,priority:string,kind:string,contact?:string|null,person?:string|null,amount?:string|null,status:string,onComplete:()=>void,onPostpone:()=>void,onPrioritize:()=>void,onEdit:()=>void,onDelete:()=>void}){
  const closed=status==='done'||status==='cancelled'
  return <article className={`record-row work-row status-${status}`}><div className={`list-icon ${isOverdue(due)&&!closed?'danger':''}`}>{isOverdue(due)&&!closed?<Clock3 size={18}/>:amount?<CircleDollarSign size={18}/>:<ClipboardList size={18}/>}</div><div className="record-main"><div className="record-title"><strong>{title}</strong><div className="record-badges"><span className={`mini-status priority-${priority}`}>{priority==='urgent'?'Urgente':priority==='high'?'Alta':kind}</span>{amount&&<span className="mini-status">{amount}</span>}{closed&&<span className="mini-status">{status==='done'?'Concluído':'Cancelado'}</span>}</div></div><div className="record-meta"><span>{formatDateTime(due)}</span>{contact&&<span>{contact}</span>}{person&&<span>Pessoa: {person}</span>}<span>{kind}</span></div>{detail&&<p>{detail}</p>}</div><div className="row-actions"><button className="secondary-button" onClick={onEdit}><Pencil size={15}/>Editar</button>{!closed&&<><button className="secondary-button" onClick={onPostpone}><Clock3 size={15}/>Amanhã</button><button className="secondary-button" onClick={onPrioritize}>Prioridade ↑</button><button className="secondary-button quick-success" onClick={onComplete}><Check size={16}/>Concluir</button></>}<button className="icon-button small danger" onClick={onDelete} title="Excluir"><Trash2 size={16}/></button></div></article>
}
