...
import { AlertTriangle, CalendarDays, ClipboardList, Mail, MessageCircle, Phone, Plus, Search, Sparkles, Trash2, UserRound, X } from 'lucide-react'
...
export function ClientsPage(){
  ...
  const load=useCallback(async(silent=false)=>{if(!currentCompany)return;if(!silent)setLoading(true);setError('');const{data,error:err}=await supabase.from('contacts').select('*').eq('company_id',currentCompany.id).order('name',{ascending:true,nullsFirst:false});if(err)setError(err.message);else setItems((data??[]) as Contact[]);if(!silent)setLoading(false)},[currentCompany])
  useEffect(()=>{void load()},[load])
  useOperationalAutoRefresh(currentCompany?.id,()=>load(true),['contacts','contact_people'])
  ...
  async function resolveSharedNumber(e:MouseEvent,item:Contact,action:'same_person'|'shared_number'|'ignore'){
    e.stopPropagation();if(!currentCompany)return
    setError('')
    const names=item.identified_names??[]
    const latest=names.at(-1)??item.name
    const payload=action==='same_person'
      ?{name:latest,identified_names:latest?[latest]:[],shared_number_suspected:false,shared_number_confirmed:false,identity_alert:null}
      :action==='shared_number'
        ?{shared_number_suspected:false,shared_number_confirmed:true,identity_alert:`Número compartilhado confirmado: ${names.join(', ')}`}
        :{shared_number_suspected:false,shared_number_confirmed:false,identity_alert:null}
    const{error:err}=await supabase.from('contacts').update(payload).eq('id',item.id).eq('company_id',currentCompany.id)
  ...
  return <section>
    ...
    <div className="cards-grid clients-grid">{loading?<div className="panel-card">Carregando...</div>:filtered.length?filtered.map(item=><article className="client-card" key={item.id} onClick={()=>void loadDetails(item)}><div className="client-avatar">{(item.name?.[0]||'?').toUpperCase()}</div><div className="client-content"><strong>{item.name||'Cliente sem nome'}</strong><div className="client-lines">{item.phone&&<span><Phone size={14}/>{item.phone}</span>}{item.email&&<span><Mail size={14}/>{item.email}</span>}{item.whatsapp_id&&<span><MessageCircle size={14}/>Contato identificado</span>}{item.shared_number_suspected&&<div className="form-error"><span><AlertTriangle size={14}/>Possível número compartilhado: {(item.identified_names??[]).join(', ')}</span><div className="heading-actions"><button type="button" className="secondary-button" onClick={e=>void resolveSharedNumber(e,item,'same_person')}>É a mesma pessoa</button><button type="button" className="secondary-button" onClick={e=>void resolveSharedNumber(e,item,'shared_number')}>Número compartilhado</button><button type="button" className="secondary-button" onClick={e=>void resolveSharedNumber(e,item,'ignore')}>Ignorar</button></div></div>}{item.shared_number_confirmed&&<span><AlertTriangle size={14}/>Número compartilhado confirmado: {(item.identified_names??[]).join(', ')}</span>}</div>{item.notes&&<p>{item.notes}</p>}<small className="client-open-hint">Abrir histórico e memória</small></div></article>):<div className="big-empty panel-card full-span"><UserRound size={34}/><h3>Nenhum cliente cadastrado</h3><p>Cadastre manualmente agora. Depois, novos contatos também poderão nascer das conversas.</p><button className="primary-button action-button" onClick={openNew}><Plus size={17}/>Cadastrar cliente</button></div>}</div>
