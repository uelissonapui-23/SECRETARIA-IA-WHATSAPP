...
export function DeviceAlertsPanel(){
  ...
  const[testStatus,setTestStatus]=useState<'idle'|'busy'|'sent'|'error'>('idle'),[testError,setTestError]=useState('')
  ...
  async function testPush(){
    ...
    setTestStatus('busy');setTestError('')
    const{data,error}=await supabase.rpc('create_push_test_notification',{target_company_id:currentCompany.id})
    if(error||!data){setTestError(error?.message||'O banco não criou o alerta de teste.');setTestStatus('error');return}
    setTestStatus('sent')
  }
  return <div className="settings-card colorful-card device-alert-card">
    ...
    {testStatus==='error'&&<p className="form-error">Não foi possível enviar o alerta de teste: {testError}</p>}
