...
export function PilotHostedConnector({ companyId, canManage }: { companyId:string; canManage:boolean }) {
  ...
  const [pollingPaused,setPollingPaused]=useState(false),[checked,setChecked]=useState(false)
  ...
  useEffect(()=>{setChecked(false);setState(empty);setError('');setPollingPaused(false)},[companyId])
  useEffect(()=>{if(pollingPaused)return;void refresh(true);const id=window.setInterval(()=>void refresh(true),!checked||state.status==='qr_ready'||state.status==='connecting'?2500:10000);return()=>window.clearInterval(id)},[checked,pollingPaused,refresh,state.status])
  ...
  return <div className="validation-card validation-simulator">
    ...
    {refreshNotice && <div className="form-success pilot-refresh-success"><CheckCircle2 size={16}/>{refreshNotice}</div>}
    <div className="validation-actions">{canManage&&checked&&!connected&&<button type="button" className="primary-button" onClick={()=>void connect()} disabled={busy||refreshing}><Link2 size={17}/>{busy?'Preparando...':'Conectar WhatsApp piloto'}</button>}<button type="button" className="secondary-button" onClick={()=>void refresh(false)} disabled={busy||refreshing} aria-busy={refreshing}><RefreshCw size={17} className={refreshing?'spin':''}/>{refreshing?'Atualizando...':'Atualizar'}</button>{canManage&&connected&&<button type="button" className="danger-button" onClick={()=>void disconnect()} disabled={busy||refreshing}><Unplug size={17}/>Desconectar e apagar sessão</button>}</div>
