...
export function PilotHostedConnector({ companyId, canManage }: { companyId:string; canManage:boolean }) {
  ...
  const refresh=useCallback(async()=>{ if(!pilotGatewayConfigured()) return; try{setState(await getPilotStatus(companyId));setError('')}catch(e){setError(e instanceof Error?e.message:'gateway_error')} },[companyId])
  ...
  return <div className="validation-card validation-simulator">
    ...
    <div className="validation-actions">{canManage&&!connected&&<button type="button" className="primary-button" onClick={()=>void connect()} disabled={busy}><Link2 size={17}/>{busy?'Preparando...':'Conectar WhatsApp piloto'}</button>}<button type="button" className="secondary-button" onClick={()=>void refresh()} disabled={busy}><RefreshCw size={17}/>Atualizar</button>{canManage&&connected&&<button type="button" className="danger-button" onClick={()=>void disconnect()} disabled={busy}><Unplug size={17}/>Desconectar e apagar sessão</button>}</div>
