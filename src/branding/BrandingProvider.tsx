...
export function BrandingProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{
    ...
    const appRoot=new URL('/',window.location.origin).href;const manifest={name:value.app_name,short_name:value.short_name,display:'standalone',start_url:appRoot,scope:appRoot,theme_color:'#0f172a',background_color:'#f8fafc',icons:[value.app_icon_192_url&&{src:new URL(value.app_icon_192_url,window.location.origin).href,sizes:'192x192',type:'image/png'},value.app_icon_512_url&&{src:new URL(value.app_icon_512_url,window.location.origin).href,sizes:'512x512',type:'image/png'}].filter(Boolean)};const blob=URL.createObjectURL(new Blob([JSON.stringify(manifest)],{type:'application/manifest+json'}));let link=document.querySelector<HTMLLinkElement>('link[rel="manifest"]');if(!link){link=document.createElement('link');link.rel='manifest';document.head.appendChild(link)}link.href=blob;return()=>URL.revokeObjectURL(blob)
  },[value])
